...
class PelangganAdmin(BaseModelAdmin):
    ...
    def total_belanja_admin(self, obj):
        ...
        return format_rupiah(total_spending)
    ...
    def process_set_diskon(self, request, pelanggan_id):
        ...
        # Create or update discount for the customer based on new rules
        # If birthday and loyal (>= 5M spending), create discount
        # If birthday but not loyal, create discount
        if is_ultah:
            diskon, created = DiskonPelanggan.objects.get_or_create(
                idPelanggan=pelanggan,
                ...
                defaults={
                    ...
                    'status': 'aktif',
                    'pesan': f'Diskon ulang tahun untuk pelanggan {pelanggan.nama_pelanggan}',
                    'scope_diskon': 'ALL_PRODUCTS',  # Ensure this applies to all products
                    'berlaku_sampai': timezone.now() + timedelta(hours=24)  # 24 hour validity
                }
            )
            ...
            if not created:
                ...
                diskon.persen_diskon = 10
                diskon.status = 'aktif'
                diskon.scope_diskon = 'ALL_PRODUCTS'  # Ensure this applies to all products
                diskon.berlaku_sampai = timezone.now() + timedelta(hours=24)  # 24 hour validity
                diskon.pesan = f'Diskon ulang tahun diperbarui untuk pelanggan {pelanggan.nama_pelanggan}'
                diskon.save()
            ...
            messages.error(request, f"{pelanggan.nama_pelanggan} tidak memiliki ulang tahun hari ini.")
            return redirect("admin:admin_dashboard_pelanggan_changelist")
...
class ProdukAdmin(BaseModelAdmin):
    list_display = ['nama_produk', 'kategori', 'harga_produk_formatted', 'stok_produk', 'get_actions_links']
    ...
    def harga_produk_formatted(self, obj):
        return format_rupiah(obj.harga_produk)
...
class TransaksiAdmin(BaseModelAdmin):
    list_display = ['nomor', 'idPelanggan', 'tanggal', 'status_transaksi_interactive', 'total_display', 'ongkir_formatted', 'bukti_bayar_display', 'bukti_refund_display', 'catatan_pembatalan_display', 'combined_actions']
    ...
    readonly_fields = ['get_nama_bank', 'get_nomor_rekening']
    
    @admin.display(description='Nama Bank Pelanggan')
    def get_nama_bank(self, obj):
        return obj.idPelanggan.nama_bank if obj.idPelanggan.nama_bank else '-'
    
    @admin.display(description='Nomor Rekening Pelanggan')
    def get_nomor_rekening(self, obj):
        return obj.idPelanggan.nomor_rekening if obj.idPelanggan.nomor_rekening else '-'
    list_per_page = 6
    
    def get_fieldsets(self, request, obj=None):
        # Define organized fieldsets for better UI
        main_info_fieldset = (
            '📦 Informasi Utama', {
                'fields': ['tanggal', 'idPelanggan', 'status_transaksi']
            }
        )
        
        payment_details_fieldset = (
            '💰 Rincian Pembayaran', {
                'fields': ['total', 'ongkir', 'bukti_bayar', 'waktu_checkout', 'batas_waktu_bayar']
            }
        )
        
        bank_info_fieldset = (
            '🏦 Data Bank Pelanggan', {
                'fields': ['get_nama_bank', 'get_nomor_rekening'],
                'classes': ['readonly'],
            }
        )
        
        shipping_refund_fieldset = (
            '🚚 Pengiriman & Refund', {
                'fields': ['alamat_pengiriman', 'foto_pengiriman', 'bukti_refund']
            }
        )
        
        cancellation_fieldset = (
            '⚠️ Pembatalan', {
                'fields': ['catatan_pembatalan'],
                'classes': ['collapse'],  # Collapsed by default
            }
        )
        
        # Combine all fieldsets
        fieldsets = [main_info_fieldset, payment_details_fieldset, bank_info_fieldset, shipping_refund_fieldset, cancellation_fieldset]
    ...
    @admin.display(description='Bukti Refund')
    def bukti_refund_display(self, obj):
        if obj.bukti_refund:
            # Check if it's an image file based on extension
            url = obj.bukti_refund.url
            if url.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp', '.webp')):
                return format_html(
                    '<a href="{}" target="_blank"><img src="{}" style="max-height: 50px; max-width: 50px;" /></a>',
                    url, url
    ...
    @admin.display(description='Ongkir')
    def ongkir_formatted(self, obj):
        return format_rupiah(obj.ongkir)

    @admin.display(description='Catatan Pembatalan')
    def catatan_pembatalan_display(self, obj):
        if obj.status_transaksi == 'DIBATALKAN' and getattr(obj, 'catatan_pembatalan', None):
            return getattr(obj, 'catatan_pembatalan', '-')
        return "-"
    ...
    def ubah_status_dibatalkan(self, request, queryset):
        ...
        for transaksi in queryset:
            try:
                ...
                # Note: Notification for cancellation is now handled centrally in signals.py
                messages.info(request, f"Notifikasi pembatalan sedang diproses oleh sistem untuk pesanan #{transaksi.id}.")
            except Exception as e:
                print(f"ERROR saat memproses pembatalan pesanan #{transaksi.id}: {str(e)}")
                failed_count += 1
        
        if failed_count > 0:
            self.message_user(request, f"{updated_count} transaksi berhasil dibatalkan. {failed_count} gagal diproses.")
        else:
            self.message_user(request, f"{updated_count} transaksi berhasil diubah statusnya menjadi Dibatalkan.")
        
        if updated_count > 0:
            messages.info(request, f"Proses pembatalan massal selesai. Notifikasi sedang diproses untuk {updated_count} pesanan.")
    ...
    def save_model(self, request, obj, form, change):
        ...
        # Check if status is changing to DIBATALKAN and there's no cancellation reason
        if obj.status_transaksi == 'DIBATALKAN' and not obj.catatan_pembatalan:
            self.message_user(request, f"Error: Pesanan #{obj.id} tidak dapat dibatalkan tanpa alasan pembatalan. Silakan isi catatan pembatalan.", level=messages.ERROR)
            return
        
        # Check if status is changing to DIBATALKAN and there's a payment proof
        if obj.status_transaksi == 'DIBATALKAN' and obj.bukti_bayar and not obj.bukti_refund:
            self.message_user(request, f"Peringatan: Pesanan #{obj.id} dibatalkan namun bukti bayar sudah ada. Mohon upload bukti refund.", level=messages.WARNING)
        
        super().save_model(request, obj, form, change)
        
        # Note: Notification for cancellation is now handled centrally in signals.py
        # Only provide feedback to admin user
        if obj.status_transaksi == 'DIBATALKAN':
            messages.info(request, f"Notifikasi pembatalan sedang diproses oleh sistem untuk pesanan #{obj.id}.")
        ...
        if old_ongkir != obj.ongkir:
            try:
                ...
                print(f"ERROR saat mengirim notifikasi ongkir: {str(e)}")
                messages.warning(request, f"Gagal mengirim notifikasi ongkir untuk pesanan #{obj.id}.")
