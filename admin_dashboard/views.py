        ...
        if form.is_valid():
            try:
                ...
                if is_birthday:
                    ...
                    def create_birthday_discount_after_commit():
                        from datetime import timedelta
                        from django.utils import timezone
                        from .models import DiskonPelanggan
                        
                        # Create DiskonPelanggan object - paksa pembuatan diskon
                        DiskonPelanggan.objects.create(
                            idPelanggan=pelanggan,
                            persen_diskon=10,
                            status='aktif',
                            scope_diskon='CART_THRESHOLD', # Ganti ke CART_THRESHOLD karena pelanggan baru belum punya riwayat belanja
                            pesan='Hadiah Ulang Tahun: Diskon 10% berlaku 24 jam!',
                            berlaku_sampai=timezone.now() + timedelta(hours=24)
                        )
                        
                        # Debug message
                        print(f"DATABASE CHECK: Record diskon dibuat untuk ID {pelanggan.id}")
                        
                        # Trigger notification via Celery
                        from .tasks import send_notification_task
                        send_notification_task.delay(
                            pelanggan_id=pelanggan.id,
                            tipe_pesan="Voucher Ulang Tahun",
                            isi_pesan="Selamat ulang tahun! Voucher diskon 10% untuk semua produk telah dibuat dan berlaku selama 24 jam."
                        )
                    
                    transaction.on_commit(create_birthday_discount_after_commit)
...
def batalkan_pesanan(request, pesanan_id):
    """
    Fungsi untuk membatalkan pesanan dengan alasan pembatalan
    """
    if request.method == 'POST':
        transaksi = get_object_or_404(Transaksi, pk=pesanan_id)
        pelanggan_id = request.session.get('pelanggan_id')
        
        # Hanya bisa dibatalkan jika statusnya masih DIPROSES dan dimiliki oleh pelanggan ini
        if transaksi.status_transaksi != 'DIPROSES' or transaksi.idPelanggan_id != pelanggan_id:
            messages.error(request, 'Pesanan hanya bisa dibatalkan saat status masih DIPROSES dan milik Anda.')
            return redirect('daftar_pesanan')
        
        # Ambil alasan pembatalan dari form
        alasan_pembatalan = request.POST.get('catatan_pembatalan', '').strip()
        
        # Update status dan tambahkan catatan pembatalan
        transaksi.status_transaksi = 'DIBATALKAN'
        transaksi.catatan_pembatalan = alasan_pembatalan
        transaksi.save()
        ...
        create_notification(
            ...
            "Pesanan Dibatalkan", 
            f"Pesanan #{transaksi.id} telah dibatalkan. Alasan: {alasan_pembatalan if alasan_pembatalan else 'Tanpa alasan'}"
        )
        
        messages.success(request, f'Pesanan #{transaksi.id} berhasil dibatalkan.')
        return redirect('daftar_pesanan')
    
    # Jika bukan POST request, kembalikan ke halaman sebelumnya
    return redirect('daftar_pesanan')
...
def update_status_transaksi(request, transaksi_id):
    """
    Fungsi admin untuk update status transaksi termasuk pembatalan
    """
    if request.method == 'POST':
        transaksi = get_object_or_404(Transaksi, pk=transaksi_id)
        status_baru = request.POST.get('status_transaksi')
        
        # Validasi status transaksi
        valid_statuses = [choice[0] for choice in STATUS_TRANSAKSI_CHOICES]
        if status_baru not in valid_statuses:
            messages.error(request, 'Status transaksi tidak valid.')
            return redirect('admin_transaksi_list')
        
        # Jika status adalah DIBATALKAN, perlu alasan pembatalan
        if status_baru == 'DIBATALKAN':
            alasan_pembatalan = request.POST.get('catatan_pembatalan', '').strip()
            if not alasan_pembatalan:
                messages.error(request, 'Alasan pembatalan diperlukan untuk pembatalan pesanan.')
                return redirect('admin_transaksi_list')
        ...
        if status_baru == 'DIBATALKAN':
            # Pesan khusus untuk pembatalan oleh admin
            nama_bank = getattr(transaksi.idPelanggan, 'nama_bank', 'N/A')
            nomor_rekening = getattr(transaksi.idPelanggan, 'nomor_rekening', 'N/A')
            
            isi_pesan = (
                f"Pesanan #{transaksi.id} telah DIBATALKAN oleh Admin. "
            ...
            create_notification(
                ...
                isi_pesan,
                url_target=f"/pesanan/{transaksi.id}/"
            )
        else:
            create_notification(
                transaksi.idPelanggan,
                "Status Pesanan Diupdate",
                f"Status pesanan #{transaksi.id} telah diubah menjadi {transaksi.get_status_transaksi_display()}. {('Alasan pembatalan: ' + alasan_pembatalan) if status_baru == 'DIBATALKAN' else ''}"
            )
        
        messages.success(request, f'Status transaksi #{transaksi.id} berhasil diupdate.')
        return redirect('admin_transaksi_list')
...
def produk_list(request):
    ...
    # DETEKSI LOYALITAS & ULTAH USER LOGIN
    is_loyal = False
    is_ultah = False
    pelanggan_id = request.session.get('pelanggan_id')
    if pelanggan_id:
    ...
    from datetime import date
    today = date.today()
    is_birthday = is_ultah  # Use the new variable
    total_spending = 0
    ...
    for p in produk:
        # Cari diskon spesifik produk atau diskon umum ALL_PRODUCTS
        diskon_produk = DiskonPelanggan.objects.filter(
            idPelanggan_id=pelanggan_id,
            status='aktif',
            berlaku_sampai__gte=timezone.now()
        ).filter(Q(idProduk=p) | Q(scope_diskon='ALL_PRODUCTS')).first()
        
        # Jika USER LOYAL & ULTAH, pastikan label diskon 10% muncul
        if not diskon_produk and is_loyal and is_ultah:
            p.diskon_aktif = type('MockDiskon', (), {
                'persen_diskon': 10,
                'scope_diskon': 'ALL_PRODUCTS',
...
def keranjang(request):
    ...
    for produk_id, jumlah in keranjang_belanja.items():
        ...
        # PAKSA POTONG 10% UNTUK USER LOYAL + ULTAH
        if is_loyal and is_birthday:
            # PAKSA POTONG 10% UNTUK USER LOYAL + ULTAH
            potongan_harga = int(Decimal(str(harga_asli)) * Decimal('0.10'))
            sub_total = harga_asli - potongan_harga
            harga_setelah_diskon = sub_total
            # Gunakan diskon produk jika ada untuk informasi saja
            diskon = diskon_produk if diskon_produk else None
        else:
            # Logika normal untuk pelanggan baru
            sub_total = harga_asli
    ...
    # Skenario 1: Jika pelanggan ultah & total keranjang >= 5jt (untuk non-loyal)
    # Skenario 2: Jika pelanggan loyal & ultah, diskon sudah diaplikasikan per-item
    total_belanja = sum(item['sub_total'] for item in produk_di_keranjang)
    total_sebelum_diskon = sum(item['harga_asli'] for item in produk_di_keranjang)
    total_diskon = total_sebelum_diskon - total_belanja
    
    # Untuk pelanggan loyal, diskon sudah diaplikasikan per-item, jadi tidak perlu potongan tambahan
    # Untuk pelanggan non-loyal, cek apakah total >= 5jt untuk diskon tambahan
    if is_birthday and total_sebelum_diskon >= 5000000 and not is_loyal:
        potongan_akhir = int(total_belanja * Decimal('0.1'))
        total_setelah_diskon = total_belanja - potongan_akhir
        total_diskon += potongan_akhir
    elif is_birthday and is_loyal:
        # Untuk pelanggan loyal, total_setelah_diskon sama dengan total_belanja karena diskon sudah diaplikasikan per-item
    ...
    # Calculate kekurangan for non-loyal customers
    if is_birthday and not is_loyal:
        kekurangan = max(0, 5000000 - total_sebelum_diskon)
    else:
        kekurangan = 0
...
def proses_pembayaran(request):
        ...
        if form.is_valid():
            ...
            try:
                with transaction.atomic():
                    ...
                    from datetime import date
                    today = date.today()
                    is_ultah = (pelanggan.tanggal_lahir and pelanggan.tanggal_lahir.day == today.day and pelanggan.tanggal_lahir.month == today.month)
                    
                    # Hitung Riwayat Belanja (Hanya yang lunas)
                    from django.db.models import Sum
                    riwayat_belanja = Transaksi.objects.filter(idPelanggan=pelanggan, status_transaksi__in=['DIBAYAR', 'DIKIRIM', 'SELESAI']).aggregate(total=Sum('total'))['total'] or 0
                    
                    # Komentari semua logic P2-A, P2-B, top_products_ids, dan filter diskon produk spesifik yang rumit
                    # P1: Get top purchased products for this customer
                    # try:
                    #     top_products = pelanggan.get_top_purchased_products(limit=3)
                    #     top_products_ids = [p.id for p in top_products]
                    ...
                    # Set default values for compatibility with later code
                    is_birthday = is_ultah
                    is_loyal = riwayat_belanja >= 5000000
                    qualifies_for_p2a = is_ultah and is_loyal
                    is_p2b_eligible = False  # We're not using P2-B logic anymore
                    all_products_discount = None  # We're using new logic
                    cart_threshold_discount = None  # We're not using P2-B logic anymore
                    top_products_ids = []  # We're not using this logic anymore
                    ...
                    for produk_id_str, jumlah in adjusted_items:
                        ...
                        # Terapkan Aturan Baru di dalam Loop Produk:
                        # SKENARIO 2 (Ultah & Loyal > 5jt): Jika is_ultah DAN riwayat_belanja >= 5.000.000, maka potong harga_satuan setiap produk sebesar 10% secara langsung.
                        if is_ultah and riwayat_belanja >= 5000000:
                            # Apply 10% birthday discount for loyal customers
                    ...
                    # Terapkan Aturan Baru setelah Loop Produk (Pada total_belanja):
                    # SKENARIO 1 (Ultah Biasa): Jika is_ultah DAN total_belanja >= 5.000.000, maka potong total_belanja akhir sebesar 10%.
                    if is_ultah and total_belanja >= 5000000:
                        potongan_birthday = total_belanja * Decimal('10') / Decimal('100')
                        total_belanja -= potongan_birthday
    ...
    for produk_id_str, jumlah in keranjang_belanja.items():
        ...
        # Logika Baru: Jika is_ultah DAN riwayat_belanja >= 5.000.000 (Skenario 2), barulah terapkan potongan 10% pada harga_setelah_diskon di dalam loop.
        # Jika tidak memenuhi syarat loyal, maka harga_setelah_diskon HARUS SAMA dengan harga_asli.
        if is_ultah and riwayat_belanja >= 5000000:
            # Check for discounts for loyal birthday customers
            diskon_produk = DiskonPelanggan.objects.filter(
                idPelanggan_id=pelanggan_id,
                idProduk=produk,
                status='aktif'
            ...
            # Hapus pengecekan diskon_produk yang menerapkan diskon per-item di dalam loop tersebut untuk pelanggan yang belum loyal.
            # If not loyal birthday customer, no per-item discount should be applied
            pass
    ...
    # Sinkronisasi Total Akhir (Skenario 1):
    # Di luar loop produk (sebelum render), pastikan variabel total_belanja hanya dipotong 10% jika is_ultah DAN total_sebelum_diskon >= 5.000.000.
    if is_ultah and total_sebelum_diskon >= 5000000:
        potongan_birthday = total_belanja * Decimal('10') / Decimal('100')
        total_belanja -= potongan_birthday
        total_diskon += potongan_birthday
...
from .tables import TransaksiTable, PelangganTransaksiTable, PelangganUltahTable, PelangganDiskonTable
...
def laporan_transaksi(request):
    ...
    # Determine the report type from the filter
    tipe_laporan = request.GET.get('tipe_laporan', 'default')
    
    # Initialize the appropriate table based on report type
    if tipe_laporan == 'ringkasan_pelanggan':
        from django.db.models import Count
        # Get customers with transaction count
        customer_data = Pelanggan.objects.annotate(
            total_transaksi=Count('transaksi', 
                                filter=Q(transaksi__status_transaksi__in=['DIBAYAR', 'DIKIRIM', 'SELESAI']))
        ).filter(total_transaksi__gt=0)
        table = PelangganTransaksiTable(customer_data)
    elif tipe_laporan == 'ultah_pelanggan':
        from datetime import datetime
        current_month = datetime.now().month
        customer_data = Pelanggan.objects.filter(tanggal_lahir__month=current_month)
        table = PelangganUltahTable(customer_data)
    elif tipe_laporan == 'diskon_pelanggan':
        diskon_data = DiskonPelanggan.objects.filter(status='aktif')
        table = PelangganDiskonTable(diskon_data)
    else:
        # Default: all transactions
        filter = TransaksiFilter(request.GET, queryset=Transaksi.objects.all())
        table = TransaksiTable(filter.qs)
    
    # Set dynamic title based on report type
    if tipe_laporan == 'ringkasan_pelanggan':
        report_title = 'Ringkasan Transaksi Pelanggan'
    elif tipe_laporan == 'ultah_pelanggan':
        report_title = 'Pelanggan Ulang Tahun'
    elif tipe_laporan == 'diskon_pelanggan':
        report_title = 'Pelanggan Penerima Diskon'
    else:
        report_title = 'Data Transaksi'
    ...
    if request.GET.get('_pdf') == 'true':
        ...
        # Determine PDF title based on report type
        if tipe_laporan == 'ringkasan_pelanggan':
            pdf_title = "Laporan Ringkasan Transaksi Pelanggan - Barokah Jaya Beton"
        elif tipe_laporan == 'ultah_pelanggan':
            pdf_title = "Laporan Pelanggan Ulang Tahun - Barokah Jaya Beton"
        elif tipe_laporan == 'diskon_pelanggan':
            pdf_title = "Laporan Pelanggan Penerima Diskon - Barokah Jaya Beton"
        else:
            pdf_title = "Laporan Transaksi - Barokah Jaya Beton"
        
        # Add title
        title = Paragraph(pdf_title, title_style)
        elements.append(title)
        
        # Add print date info below the title
        from datetime import datetime
        current_date = datetime.now().strftime('%d/%m/%Y')
        date_para = Paragraph(f"Tanggal Cetak: {current_date}", ParagraphStyle('DateStyle', parent=styles['Normal'], fontSize=9, alignment=1))
        elements.append(date_para)
        elements.append(Spacer(1, 0.2*inch))
        
        # Prepare data for the table based on report type
        if tipe_laporan == 'ringkasan_pelanggan':
            table_data = [['No', 'Nama Pelanggan', 'Banyaknya Transaksi']]
            
            # Get all data without pagination for PDF
            all_data = Pelanggan.objects.annotate(
                total_transaksi=Count('transaksi', 
                                    filter=Q(transaksi__status_transaksi__in=['DIBAYAR', 'DIKIRIM', 'SELESAI']))
            ).filter(total_transaksi__gt=0)
            
            for index, customer in enumerate(all_data, 1):
                table_data.append([
                    str(index),
                    Paragraph(str(customer.nama_pelanggan), styles['Normal']),
                    str(customer.total_transaksi)
                ])
        
        elif tipe_laporan == 'ultah_pelanggan':
            table_data = [['No', 'Nama Pelanggan', 'Tanggal Lahir', 'No HP']]
            
            # Get all data without pagination for PDF
            current_month = datetime.now().month
            all_data = Pelanggan.objects.filter(tanggal_lahir__month=current_month)
            
            for index, customer in enumerate(all_data, 1):
                tanggal_lahir_formatted = customer.tanggal_lahir.strftime('%d/%m/%Y') if customer.tanggal_lahir else ''
                table_data.append([
                    str(index),
                    Paragraph(str(customer.nama_pelanggan), styles['Normal']),
            ...
            table_data = [['No', 'Nama Pelanggan', 'Persen Diskon', 'Status']]
            
            # Get all data without pagination for PDF
            all_data = DiskonPelanggan.objects.filter(status='aktif')
            
            for index, diskon in enumerate(all_data, 1):
                table_data.append([
                    str(index),
                    Paragraph(str(diskon.idPelanggan.nama_pelanggan), styles['Normal']),
                    f"{diskon.persen_diskon}%",
                    str(diskon.status)
                ])
            ...
            for index, transaksi in enumerate(all_data, 1):
                ...
                table_data.append([
                    str(index),
                    tanggal_formatted,
                    Paragraph(str(pelanggan_nama), styles['Normal']),
                    Paragraph(detail_produk_str, styles['Normal']),
                    f"Rp {transaksi.total:,.0f}" if transaksi.total else "Rp 0"
                ])
        
        # Calculate column widths for full width table
        available_width = A4[0] - 2 * inch  # A4 width minus left and right margins
        
        # Determine number of columns and distribute width accordingly
        col_count = len(table_data[0]) if table_data else 1
        col_widths = [available_width / col_count] * col_count
        
        # For the discount report, adjust column widths to better fit content
        if tipe_laporan == 'diskon_pelanggan' and col_count == 4:
            # Allocate more space to name column, less to others
            col_widths = [available_width * 0.1, available_width * 0.5, available_width * 0.2, available_width * 0.2]
        ...
        pdf_table.setStyle(TableStyle([
            ...
            ('ALIGN', (1, 0), (1, -1), 'LEFT'),     # Left align names for better readability
        ...
        # Add total pendapatan or appropriate summary based on report type
        elements.append(Spacer(1, 0.2*inch))
        
        if tipe_laporan == 'ringkasan_pelanggan':
            total_pelanggan = len(all_data)
            summary_para = Paragraph(f"<b>Total Pelanggan dengan Transaksi: {total_pelanggan}</b>", styles['Normal'])
            elements.append(summary_para)
        elif tipe_laporan == 'ultah_pelanggan':
            total_ultah = len(all_data)
            summary_para = Paragraph(f"<b>Total Pelanggan Ulang Tahun Bulan Ini: {total_ultah}</b>", styles['Normal'])
            elements.append(summary_para)
        elif tipe_laporan == 'diskon_pelanggan':
            total_diskon = len(all_data)
            summary_para = Paragraph(f"<b>Total Pelanggan dengan Diskon Aktif: {total_diskon}</b>", styles['Normal'])
            elements.append(summary_para)
        else:
            # Default: all transactions
...
def laporan_produk_terlaris(request):
    ...
    if request.GET.get('_pdf') == 'true':
        ...
        table_data = [['No', 'Nama Produk', 'Kuantitas Terjual']]
        
        # Dapatkan semua data (queryset yang sudah terfilter dan terurut)
        all_data = produk_queryset
        total_kuantitas_keseluruhan = 0
        
        for index, produk in enumerate(all_data, 1):
            # Ambil nilai total kuantitas terjual yang sudah di-annotate
            kuantitas = produk.total_kuantitas_terjual or 0
            total_kuantitas_keseluruhan += kuantitas
        ...
        # Calculate column widths for full width table
        available_width = A4[0] - 2 * inch  # A4 width minus left and right margins
        col_widths = [available_width * 0.1,  # No column - 10% of width
                      available_width * 0.7,  # Product name column - 70% of width
                      available_width * 0.2]  # Quantity column - 20% of width
        ...
        pdf_table.setStyle(TableStyle([
            ...
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
            ('ALIGN', (0, 0), (-1, -1), 'CENTER'),  # Center all content
            ('ALIGN', (1, 0), (1, -1), 'LEFT'),     # Left align product names
            ('ALIGN', (2, 0), (-1, -1), 'RIGHT'),   # Right align quantities
            ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
            ('FONTSIZE', (0, 0), (-1, 0), 10),     # Header font size
            ('FONTSIZE', (0, 1), (-1, -1), 9),     # Body font size
            ('BOTTOMPADDING', (0, 0), (-1, 0), 12),
        ...
        # Add total kuantitas keseluruhan
        elements.append(Spacer(1, 0.2*inch))
        total_kuantitas_para = Paragraph(f"<b>Total Akumulasi Kuantitas Terjual: {total_kuantitas_keseluruhan} Unit</b>", styles['Normal'])
        elements.append(total_kuantitas_para)
    ...
    # Add current date to context for print date
    from datetime import datetime
    current_date = datetime.now().strftime('%d/%m/%Y')
